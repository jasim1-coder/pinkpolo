...
export const TopBar: React.FC<TopBarProps> = ({
  ...
  const handleSimulateClick = () => {
    const newReg = simulateNewRegistration();
    // Optional: jump to registrations to show it or open modal
  ...
  return (
    <header className="h-16 px-3 sm:px-4 lg:px-6 bg-white border-b border-slate-200 flex items-center justify-between sticky top-0 z-30 min-w-0">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        ...
        {/* Auto Demo Switch Button (Visible on wide screens) */}
        <button
          type="button"
          onClick={toggleAutoSimulate}
          className={`hidden 2xl:inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
            autoSimulateEnabled
              ? 'bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-100'
              : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'
          }`}
          title="Toggle automated simulated registrations every 13 seconds"
        >
          {autoSimulateEnabled ? (
            <>
              <span className="w-2 h-2 rounded-full bg-rose-500 animate-ping" />
              <Pause className="w-3 h-3 text-rose-600" />
              <span className="whitespace-nowrap">Auto-Simulating</span>
            </>
          ) : (
            <>
